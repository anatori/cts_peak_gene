...
compute_cis_pairs : find cis pairs within 500kb of annotated peak and genes
    - Input : --job | --multiome_file | --batch_size | --links_file | --method 
    - Output : additional columns added to links_file pertaining to computed pvalues
...
def main(args):
    ...
    if JOB == "compute_pr":
        ...
        if f'{n_rna_mean}.{n_rna_var}' == 'inf.inf':
            ...
            n_links = eval_df.shape[0]

            peak_idx = adata_atac.var.index.get_loc(links_arr[0])
            gene_idx = adata_rna.var.index.get_loc(links_arr[1])
            ctrl_peaks_reidx = ctrl_peaks[peak_idx, :n_ctrl]
            ctrl_peaks_reidx = ctrl_peaks_reidx.flatten()

            ctrl_links_arr = np.column_stack(
                ctrl_peaks_reidx, 
                np.hstack([np.full(n_ctrl, gene) for gene in gene_idx])
                )
            ctrl_labels_ls = [f'ctrl_{i}.npy' for i in range(n_links)]
            ...
            ctar.parallel.multiprocess_poisson_irls_batched(
                links=ctrl_links_arr,
                labels_ls=ctrl_labels_ls,
            ...
            n_files = len(os.listdir(final_corr_path))
            if n_files == ((n_links // BATCH_SIZE) + 1):
                
                print(f"# All {n_files} batches complete. Consolidating null...")
            ...
            for ctrl_file in ctrl_files:
                ...
                ctrl_labels_ls.append(os.path.basename(ctrl_file))
            ...
            ctar.parallel.multiprocess_poisson_irls_batched(
                ...
                local_directory=dask_tmp_path,
                batch_size=BATCH_SIZE,
                save_files=False, # test
            )

            print('# BATCH Regression_time = %0.2fs' % (time.time() - start_time))

            # Once done, check that all files are saved
            final_ctrl_path = os.path.join(ctrl_path, f'ctrl_links_{BIN_CONFIG}')
            missing_bins = ctar.data_loader.check_missing_bins(final_ctrl_path, final_corr_path, prefix='poissonb_')
            ...
            if len(missing_bins) > 0:
                ...
                ctar.parallel.multiprocess_poisson_irls_batched(
                    links=ctrl_labels_ls,
                    labels_ls=file_name_ls,
                    atac_sparse=atac_sparse,
                    rna_sparse=rna_sparse,
                    out_path=final_corr_path,
                    max_iter=100,
    ...
    if JOB == "compute_cis_pairs":
        ...
        if METHOD == 'corr':
            ...
            print('# Correlation_time = %0.2fs' % (time.time() - start_time))
        ...
        if METHOD == 'pr':
            ...
            corr = ctar.parallel.multiprocess_poisson_irls_batched(links=links_arr, 
                ...
                sub_batch_size=1000, # hardcoded
                n_workers=n_jobs,
                # processes=False, # recommended when returning results
                batch_size=BATCH_SIZE,
