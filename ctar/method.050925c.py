...
def vectorized_poisson_regression2(mat_X, mat_Y, max_iter=100, tol=1e-6, flag_float32=True):
    """ Fast poisson regression from Alistair
    ...
    # if flag_float32 is True: # use np.float32
    ...
    #     else: 
    #         mat_Y_broad = mat_Y
    #     v_beta0 = np.zeros(P, dtype=np.float32)
    #     v_beta1 = np.zeros(P, dtype=np.float32)
    # else: 
    #     pass # todo: not use np.float32
    print('check 1')
    ...
    # if mat_Y.ndim == 1: mat_Y_broad = mat_Y.reshape(-1,1)
    # else: mat_Y_broad = mat_Y
