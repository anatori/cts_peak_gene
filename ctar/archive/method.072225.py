...
def process_ctrl_pr_set(ctrl_file, ctrl_path, BIN_CONFIG, n_ctrl, adata_atac, adata_rna, final_corr_path):
    ctrl_links = np.load(os.path.join(ctrl_path, f'ctrl_links_{BIN_CONFIG}', ctrl_file))[:n_ctrl]

    atac_data = adata_atac.layers['counts'][:, ctrl_links[:, 0]]
    rna_data = adata_rna.layers['counts'][:, ctrl_links[:, 1]]

    _, poisson_beta, _ = vectorized_poisson_regression_safe_converged(atac_data, rna_data, tol=1e-3)

    save_path = os.path.join(final_corr_path, f'poissonb_{os.path.basename(ctrl_file)}')
    np.save(save_path, poisson_beta.flatten())
    print(f"# Saved {save_path}")


def run_parallel_pr(ctrl_path, BIN_CONFIG, n_ctrl, adata_atac, adata_rna, final_corr_path, n_jobs=4):
    ctrl_files = os.listdir(os.path.join(ctrl_path, f'ctrl_links_{BIN_CONFIG}'))

    Parallel(n_jobs=n_jobs)(
        delayed(process_ctrl_pr_set)(f, ctrl_path, BIN_CONFIG, n_ctrl, adata_atac, adata_rna, final_corr_path)
        for f in ctrl_files
    )
