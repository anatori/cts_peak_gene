...
def poisson_irls_single_dask(mat_x, mat_y, max_iter=100, tol=1e-3):
    """
    IRLS Poisson regression on sparse COO dask arrays for a single feature pair.
    mat_x, mat_y: dask arrays chunked with sparse.COO, shape (n_cells,)
    Returns: np.array([beta0, beta1]) (computed, dense)
    """
    ...
    beta0 = da.zeros((), dtype=np.float32)  # scalar
    beta1 = da.zeros((), dtype=np.float32)  # scalar

    # Compute sums safely with sparse ops
    for _ in range(max_iter):

        eta = beta0 + mat_x * beta1
        eta = da.clip(eta, -MAX_EXP, MAX_EXP)

        mu = da.exp(eta)

        w = mu
        z = eta + (mat_y - mu) / da.maximum(w, MIN_VAL)

        sum_x = (mat_x * w).sum()
        sum_y = (w * z).sum()
        denom = (mat_x * mat_x * w).sum() - (sum_x ** 2) / w.sum()
        denom = da.maximum(denom, MIN_VAL)

        beta1_new = ((mat_x * w * z).sum() - sum_x * sum_y / w.sum()) / denom
        beta0_new = (sum_y - beta1_new * sum_x) / w.sum()

        diff_beta1, diff_beta0 = da.compute(
            da.abs(beta1_new - beta1),
            da.abs(beta0_new - beta0)
        )

        beta0 = beta0_new
        beta1 = beta1_new

        if diff_beta1 < tol and diff_beta0 < tol:
            break

    # Compute final values
    return np.array([beta0.compute(), beta1.compute()], dtype=np.float32)


def poisson_irls_delayed_sparse_loop(mat_x_full, mat_y_full, max_iter=100, tol=1e-3):
    """
    mat_x_full, mat_y_full: dask arrays with shape (n_cells, n_pairs)
    Calls poisson_irls_single_dask on each pair column independently with dask.delayed
    ...
    for i in range(n_pairs):
        # slice columns i (still sparse dask arrays with shape (n_cells,))
        x_i = mat_x_full[:, i]
        y_i = mat_y_full[:, i]

        # Use delayed wrapper to run in parallel
        res = delayed(poisson_irls_single_dask)(x_i, y_i, max_iter=max_iter, tol=tol)
        results.append(res)

    # Compute all results in parallel
    results_computed = compute(*results)
...
def poisson_irls_delayed_loop(mat_x_full, mat_y_full, max_iter=100, tol=1e-3):
    ...
    for i in range(n_pairs):
        ...
        delayed_result = delayed(poisson_irls_single)(
            x_i, y_i, max_iter=max_iter, tol=tol
        )
        results_delayed.append(delayed_result)
...
def poisson_irls_single(x, y, max_iter=100, tol=1e-3):
    ...
    for _ in range(max_iter):
        eta = np.clip(beta0 + x * beta1, -MAX_EXP, MAX_EXP)
        mu = np.exp(eta)
        
        w = mu
        z = eta + (y - mu) / np.maximum(w, MIN_VAL)

        sum_x = np.sum(x * w)
        sum_y = np.sum(w * z)
        denom = np.sum(x * x * w) - (sum_x ** 2) / np.sum(w)
        denom = max(denom, MIN_VAL)

        beta1_new = (np.sum(x * w * z) - sum_x * sum_y / np.sum(w)) / denom
        beta0_new = (sum_y - beta1_new * sum_x) / np.sum(w)
...
def vectorized_poisson_regression_dask_converged(
    mat_x, mat_y,
    max_iter=100,
    tol=1e-3,
    pct_converged=0.9,
    ...
    """
    Adapted for dask arrays backed by sparse.COO arrays.

    Parameters:
    - mat_x: dask.array with sparse.COO chunks, shape (n_cell, n_pair)
    - mat_y: dask.array with sparse.COO chunks or dense, shape (n_cell, n_pair) or (n_cell, 1)
    - max_iter, tol, pct_converged, flag_float32: as before

    Returns:
    - v_beta0_final, v_beta1_final: np.ndarray (computed results)
    - pct_converged_mask: np.ndarray boolean mask
    """

    n_cell, n_pair = mat_x.shape
    dtype = np.float32 if flag_float32 else float

    # Initialize coefficients: shape (1, n_pair)
    v_beta0 = da.zeros((1, n_pair), dtype=dtype)
    v_beta1 = da.zeros((1, n_pair), dtype=dtype)
    v_beta0_final = da.zeros((1, n_pair), dtype=dtype)
    v_beta1_final = da.zeros((1, n_pair), dtype=dtype)

    pct_converged_mask = np.zeros(n_pair, dtype=bool)

    # Broadcast mat_y if shape (n_cell, 1)
    if mat_y.shape[1] == 1:
        mat_y = da.repeat(mat_y, n_pair, axis=1)
    ...
    for iteration in tqdm(range(max_iter)):
        ...
        # Linear predictor: X*beta1 + beta0
        # mat_x * v_beta1 means elementwise multiplication along columns
        # v_beta1 shape (1, n_pair) broadcasts over rows
        sum_xy = (mat_x * v_beta1) + v_beta0  # shape (n_cell, n_pair)
        # Clip exponent to avoid overflow
        sum_xy = da.clip(sum_xy, -MAX_EXP, MAX_EXP)

        sum_w = da.exp(sum_xy)  # predicted mean, shape (n_cell, n_pair)

        # sum_x = mat_x * sum_w
        sum_x = mat_x * sum_w

        # denom = mat_x * sum_x  (elementwise multiply mat_x and sum_x)
        denom = mat_x * sum_x

        # temp = (mat_y - sum_w) / sum_w_safe
        sum_w_safe = da.maximum(sum_w, MIN_VAL)
        temp = (mat_y - sum_w) / sum_w_safe

        # update sum_xy for IRLS step
        sum_xy = sum_xy + temp

        # sum_w_safe clipped to max val
        sum_w_safe = da.clip(sum_w_safe, 0, MAX_VAL)

        sum_y = sum_w_safe * sum_xy  # elementwise
        sum_y = sum_y.sum(axis=0)    # sum over rows => shape (n_pair,)

        # denom sums
        denom_sum = denom.sum(axis=0)  # shape (n_pair,)
        sum_w_sum = sum_w.sum(axis=0)  # shape (n_pair,)
        sum_x_sum = sum_x.sum(axis=0)  # shape (n_pair,)

        # Compute denom scalar for beta update:
        denom_scalar = denom_sum - (sum_x_sum ** 2) / sum_w_sum
        denom_scalar = da.maximum(denom_scalar, MIN_VAL)

        # Clip other sums
        sum_y_safe = da.clip(sum_y, -MAX_VAL, MAX_VAL)
        sum_x_safe = da.clip(sum_x_sum, -MAX_VAL, MAX_VAL)
        sum_w_safe_sum = da.maximum(sum_w_sum, MIN_VAL)
        denom_safe = denom_scalar

        # Compute new beta1:
        v_beta1_new = sum_xy - (sum_x_safe * sum_y_safe) / sum_w_safe_sum
        v_beta1_new = da.clip(v_beta1_new, -MAX_VAL, MAX_VAL) / denom_safe

        # Compute new beta0:
        v_beta0_new = (sum_y_safe - v_beta1_new * sum_x_safe) / sum_w_safe_sum
        v_beta0_new = da.clip(v_beta0_new, -MAX_VAL, MAX_VAL)

        converged_mask = ((da.abs(v_beta1_new - v_beta1) < tol) &
                          (da.abs(v_beta0_new - v_beta0) < tol))
        converged_fraction = converged_mask.mean().compute()

        if not is_pct_unconverged and (converged_fraction >= pct_converged):
            print(f"{pct_converged*100}% pairs converged after {iteration+1} iterations.")
            is_pct_unconverged = True

            # Store converged results
            v_beta0_final = v_beta0_final.compute()
            v_beta1_final = v_beta1_final.compute()
            v_beta0_new = v_beta0_new.compute()
            v_beta1_new = v_beta1_new.compute()

            v_beta0_final[:, converged_mask] = v_beta0_new[:, converged_mask]
            v_beta1_final[:, converged_mask] = v_beta1_new[:, converged_mask]

            # Update variables for unconverged only
            unconverged_idx = ~converged_mask
            v_beta0_new = v_beta0_new[:, unconverged_idx]
            v_beta1_new = v_beta1_new[:, unconverged_idx]

            # Filter columns in mat_x and mat_y for unconverged
            mat_x = mat_x[:, unconverged_idx]
            mat_y = mat_y[:, unconverged_idx]

            v_beta0 = da.from_array(v_beta0_new, chunks=(1, v_beta0_new.shape[1]))
            v_beta1 = da.from_array(v_beta1_new, chunks=(1, v_beta1_new.shape[1]))

            pct_converged_mask = converged_mask.copy()
            n_pair = v_beta0.shape[1]

        if np.all(converged_mask) or iteration == (max_iter - 1):
            print(f"Converged after {iteration+1} iterations.")

            if is_pct_unconverged:
                # Compute final beta arrays to numpy
                v_beta0_final[:, ~pct_converged_mask] = v_beta0_new.compute()
                v_beta1_final[:, ~pct_converged_mask] = v_beta1_new.compute()
            else:
                v_beta0_final = v_beta0_new.compute()
                v_beta1_final = v_beta1_new.compute()
