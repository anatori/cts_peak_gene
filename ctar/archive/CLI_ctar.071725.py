...
def main(args):
    ...
    if JOB == "compute_pr":
        ...
        if f'{n_rna_mean}.{n_rna_var}' == 'inf.inf':
            ...
            mini_batch_size = 50 # hard coded
            mini_batch = ctrl_links_full.shape[0] // mini_batch_size

            for i in range(mini_batch_size):

                start = i * mini_batch
                end = (i + 1) * mini_batch if i < mini_batch_size - 1 else ctrl_links_full.shape[0]

                atac_data = adata_atac.layers['counts'][:, ctrl_links_full[start:end, 0]]
                rna_data = adata_rna.layers['counts'][:, ctrl_links_full[start:end, 1]]

                _, ctrl_poissonb_i, _ = ctar.method.vectorized_poisson_regression_safe_converged(atac_data, rna_data, tol=1e-3)
                ctrl_poissonb.append(ctrl_poissonb_i.flatten())

            print('# Regression_time = %0.2fs' % (time.time() - start_time))

            ctrl_poissonb = np.concatenate(ctrl_poissonb)
            print(ctrl_poissonb.shape)

            for i,file_name in enumerate(file_name_ls):
                print(f"# Saving poissonb_{file_name} to {final_corr_path}")
                np.save(os.path.join(final_corr_path, f'poissonb_{file_name}'), ctrl_poissonb[i*n_ctrl:(i+1)*n_ctrl].flatten())
