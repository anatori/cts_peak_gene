...
@njit
def _poisson_regression_core1(x, y, max_iter=100, tol=1e-3):
    n_cell, n_pair = x.shape

    beta0 = np.zeros((1, n_pair), dtype=np.float32)
    beta1 = np.zeros((1, n_pair), dtype=np.float32)
    ...
    for it in range(max_iter):
        ...
        xz = x * w * z
        xz_sum = np.sum(xz, axis=0)
        wz_sum = np.sum(w * z, axis=0)

        denom = np.sum(x * x * w, axis=0) - (xw_sum ** 2) / w_sum
        denom = np.maximum(denom, MIN_VAL)

        b1_new = (xz_sum - xw_sum * wz_sum / w_sum) / denom
        b0_new = (wz_sum - b1_new * xw_sum) / w_sum

        delta0 = np.abs(b0_new - beta0[0])
        delta1 = np.abs(b1_new - beta1[0])

        if np.all(delta0 < tol) and np.all(delta1 < tol):
            break
...
@njit
def _poisson_regression_core(x, y, max_iter=100, tol=1e-3, pct_converged=0.9):
    n_cell, n_pair = x.shape

    # Storage for final betas
    beta0_final = np.zeros((1, n_pair), dtype=np.float32)
    beta1_final = np.zeros((1, n_pair), dtype=np.float32)
    
    # Mask for pairs finished
    finished_mask = np.zeros(n_pair, dtype=np.bool_)

    # Start with all pairs active
    active_idx = np.arange(n_pair)

    beta0 = np.zeros((1, x.shape[1]), dtype=np.float32)
    beta1 = np.zeros((1, x.shape[1]), dtype=np.float32)
    ...
    for it in range(max_iter):
        ...
        converged_mask = (delta0 < tol) & (delta1 < tol)

        # Store finished betas to final arrays (use active_idx to place correctly)
        for i in range(len(active_idx)):
            if converged_mask[i]:
                idx = active_idx[i]
                beta0_final[0, idx] = b0_new[i]
                beta1_final[0, idx] = b1_new[i]

        # Keep only non-converged pairs for next iteration
        if np.all(converged_mask):
            # All finished
            break

        # Update arrays for next iteration
        active_idx = active_idx[~converged_mask]
        x = x[:, ~converged_mask]
        y = y[:, ~converged_mask]
        beta0 = b0_new[~converged_mask].reshape(1, -1)
        beta1 = b1_new[~converged_mask].reshape(1, -1)

    # After finishing, for any pairs not converged till max_iter, store final results
    for i in range(len(active_idx)):
        idx = active_idx[i]
        beta0_final[0, idx] = beta0[0, i]
        beta1_final[0, idx] = beta1[0, i]

    return beta0_final, beta1_final


@njit(parallel=True)
def batch_poisson_regression(xs, ys, max_iter=100, tol=1e-3):
    n_batches = len(xs)
    beta0s = [np.zeros((1, xs[0].shape[1]), dtype=np.float32) for _ in range(n_batches)]
    beta1s = [np.zeros((1, xs[0].shape[1]), dtype=np.float32) for _ in range(n_batches)]

    for i in prange(n_batches):
        beta0s[i], beta1s[i] = _poisson_regression_core(xs[i], ys[i], max_iter, tol)
