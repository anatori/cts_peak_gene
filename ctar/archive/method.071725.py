...
def vectorized_poisson_regression_safe_adaptive(mat_x, mat_y, max_iter=100, tol=1e-3, pct_converged=0.5, flag_float32=True):
    ...
    v_beta0_final = np.matrix(np.full((1, n_pair), np.nan))
    v_beta1_final = np.matrix(np.full((1, n_pair), np.nan))
    ...
    for iteration in range(max_iter):
        ...
        if np.all(converged_mask) or iteration == (max_iter - 1):
            ...
            nan_mask = np.isnan(v_beta0_final)
            v_beta0_final.A[nan_mask] = v_beta0_new.A.flatten()
            v_beta1_final.A[nan_mask] = v_beta1_new.A.flatten()

            break

        if (np.sum(converged_mask) / n_pair) >= pct_converged:
            print(f"{np.sum(converged_mask)} pairs converged after {iteration+1} iterations.")
    ...
    return np.array(v_beta0_final), np.array(v_beta1_final)
