...
def poisson_irls_multi_numba(X, y, max_iter=100, tol=1e-3, lambda_reg=0.0):
    ...
    WX = np.empty((n, p), dtype=np.float32)
    XtWX = np.empty((p, p), dtype=np.float32)
    XtWz = np.empty(p, dtype=np.float32)
    sum_x = np.empty(p, dtype=np.float32)
    ...
    for _ in range(max_iter):
        ...
        # z = eta + (y - mu) / mu
        for i in range(n):
            denom = mu[i] if mu[i] > MIN_VAL else MIN_VAL
            z[i] = eta[i] + (y[i] - mu[i]) / denom
        ...
        for j in range(p):
            XtWX[j, j] += MIN_VAL
        betas_new = solve_float32(XtWX, XtWz)
