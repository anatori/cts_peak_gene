...
def test_vectorized_poisson_regression():
    ...
    v_beta0_true = np.zeros((50,))
    v_beta1_true = np.zeros((50,))
    ...
    assert np.allclose(v_beta0, v_beta0_true, rtol=1e-4, equal_nan=True), err_msg
    ...
    assert np.allclose(v_beta1, v_beta1_true, rtol=1e-4, equal_nan=True), err_msg
